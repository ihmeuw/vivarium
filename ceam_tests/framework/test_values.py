...
from ceam.framework.values import replace_combiner, list_combiner, set_combiner, joint_value_combiner, rescale_post_processor, joint_value_post_processor, Pipeline, ValuesManager, NullValue
...
def test_joint_value():
    ...
    manager = ValuesManager()
    manager.declare_pipeline('test', combiner=list_combiner, post_processor=joint_value_post_processor, source=lambda index: [pd.Series(0, index)])
