...
class DiseaseTransition(Transition):
    def __init__(
        self, input_state, output_state, cause_key, measure, rate_name, **kwargs
    ):
        super().__init__(
...
class DiseaseState(State):
    ...
    def setup(self, builder):
        ...
        builder.value.register_value_modifier(
            "mortality_rate", self.add_in_excess_mortality
        )
        self.population_view = builder.population.get_view(
            [self._model],
            query=f"alive == 'alive' and {self._model} == '{self.state_id}'",
        )

    def add_transition(self, output, measure, rate_name, **kwargs):
        t = DiseaseTransition(
            self, output, self.cause_key, measure, rate_name, **kwargs
        )
    ...
    def add_in_excess_mortality(self, index, mortality_rates):
        ...
        mortality_rates.loc[affected.index] += self.excess_mortality_rate(
            affected.index
        )
...
class DiseaseModel(Machine):
    def __init__(
        self, disease, initial_state, cause_specific_mortality_rate=0.0, **kwargs
    ):
    ...
    def setup(self, builder):
        ...
        self.cause_specific_mortality_rate = builder.value.register_rate_producer(
            ...
            source=lambda index: pd.Series(
                self._cause_specific_mortality_rate, index=index
            ),
        )
        builder.value.register_value_modifier(
...
class SIS_DiseaseModel:
    ...
    def __init__(self, disease_name):
        ...
        self.configuration_defaults = {
            disease_name: SIS_DiseaseModel.configuration_defaults["disease"]
        }
