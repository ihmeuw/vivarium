...
class ScalarTable:
    """A callable that returns a series or dataframe from a single value or list
    of values with no interpolation.
    ...
    def __call__(self, index: pd.Index) -> pd.DataFrame:
        """Build a table with ``index`` and columns for each of the scalar
        values.
...
class LookupTable:
    ...
    def __call__(self, index: pd.Index) -> Union[pd.DataFrame, pd.Series]:
...
class LookupTableInterface:
    ...
    def build_table(self, data,
        ...
        specified order will be calculated for each permutation of the set of
        key_columns. The columns in parameter_columns will be used as parameters
        for the interpolation functions which will estimate all remaining
        columns in the table.

        If data is a number, time, list, or tuple, a scalar table will be
        constructed with the values in data as the values in each column of the
        table, named according to value_columns.
        ...
        data
            ...
            LookupTable.
        key_columns
            Columns used to select between interpolation functions. These should
            be the non-continuous variables in the data. For example 'sex' in
