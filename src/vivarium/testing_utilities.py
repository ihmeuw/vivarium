...
class TestPopulation(NonCRNTestPopulation):
    ...
    def generate_test_population(self, pop_data):
        ...
        if age_start == age_end:
            age = (
                age_draw * (pop_data.creation_window / pd.Timedelta(days=365))
                + age_start
            )
        else:
...
def _non_crn_build_population(
    ...
    if age_start == age_end:
        age = (
            randomness_stream.get_draw(index)
            * (creation_window / pd.Timedelta(days=365))
            + age_start
        )
    else:
...
def make_dummy_column(name, initial_value):
    class DummyColumnMaker:
        ...
        def setup(self, builder):
            ...
            builder.population.initializes_simulants(
                self.make_column, creates_columns=[name]
            )
