...
def test_incidence_rate():
    ...
    assert_rate(simulation, lambda sim: (sim.population.no_initial_disease == True).sum(), lambda sim: (sim.population.no_initial_disease == False).sum(), 0.7)
...
def test_mortality_rate():
    ...
    assert_rate(simulation, lambda sim: (sim.population.alive == False).sum(), lambda sim: (sim.population.alive == True).sum(), 0.7)
