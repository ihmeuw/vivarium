...
@pytest.mark.skip(
    reason="only order 0 interpolation with age bin edges currently supported"
)
def test_interpolated_tables(base_config):
    ...
    simulation = InteractiveContext(
        components=[TestPopulation()], configuration=base_config
    )
...
def test_interpolated_tables__exact_values_at_input_points(base_config):
    ...
    for year in input_years:
        ...
        assert np.allclose(
            years(simulation.get_population().index),
