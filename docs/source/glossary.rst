...
.. glossary::
    ...
    Attribute Pipeline
        ...
        a simulant index and return tabular data of :term:`attributes <Attribute>`
        for those simulants.
    ...
    Private Column
        ...
        Only the component that creates a private column has read or write access
        to it.

    Public Column
        A column in the population state table that is generated by an 
        :term:`attribute pipeline <Attribute Pipeline>` and is read-accessible by
        all :term:`components <Component>`. A public column is not directly
        writable; instead, it can be modified by updating the corresponding
        :term:`private column <Private Column>` or by registering a modifer to
        the attribute pipeline that generated it.
